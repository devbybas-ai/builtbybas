...
> **Last Updated:** 2026-03-15 (Session 57)
> **Status:** LIVE -- builtbybas.com operational. Milestone Billing System IMPLEMENTED.
> **Next Session Priority:** Deploy billing system to VPS. Run Drizzle migration on production DB. Add CRON_SECRET to production .env.local. Set up VPS crontab for daily billing sweep. Rotate Resend API key (ISS-12).

## Session 57 Changes (2026-03-15)

**Milestone Billing System -- IMPLEMENTED (17 tasks, 15 commits):**

Full 50/25/25 billing system built from approved spec. 274 tests passing, build clean.

**Chunk 1 -- Data Layer:**
- New `billing_milestones` table with project/invoice FKs, type/status enums, 3 indexes
- New columns on `projects` (billingModel, proposalId) and `invoices` (token, reminderSentAt, milestoneId, paymentMethod)
- Billing utility library (TDD, 16 tests): calculateMilestoneAmounts, mapTimelineToWeeks, calculateMilestoneDates, generateInvoiceToken, buildDepositLineItems
- Billing types, validation schemas, env config (CRON_SECRET, Stripe vars)
- Extracted `generateInvoiceNumber` to shared `src/lib/invoice-utils.ts`

**Chunk 2 -- Proposal Accept Flow:**
- `src/app/api/proposals/respond/route.ts` -- on accept: auto-creates project, 3 milestones (50/25/25), deposit invoice with line items, sends deposit email to client
- Idempotency: proposalId unique constraint prevents duplicate project creation
- All within single DB transaction, email post-commit

**Chunk 3 -- Invoice Email + Client Page:**
- `src/lib/invoice-email.ts` -- 3 branded email templates (invoice, payment reminder, milestone alert)
- `POST /api/invoices/[id]/send` -- send/resend invoice email, rotates token, updates milestone status
- `GET /api/invoices/view?token=` -- public token-based invoice data endpoint
- `/invoice/[token]` -- client-facing invoice page (glassmorphism, status banners, policy footer)

**Chunk 4 -- Cron Worker:**
- `src/lib/cron-auth.ts` -- timing-safe bearer token + admin session fallback
- `POST /api/cron/billing` -- 5 sweep operations: milestone draft generation, overdue detection, payment reminders (3 days), upcoming alerts (5 days), admin notifications

**Chunk 5 -- Stripe Framework (Dormant):**
- `src/lib/stripe.ts` -- isStripeEnabled() helper
- `POST /api/invoices/[id]/pay` -- returns 503 until Stripe configured
- `POST /api/stripe/webhook` -- skeleton for checkout.session.completed handling

**Chunk 6 -- Admin UI:**
- `src/components/admin/BillingTimeline.tsx` -- visual 3-node horizontal timeline with status-colored connectors
- Project detail page shows billing timeline with milestones
- Invoice detail: send/resend button, mark-paid with payment method selector, milestone type badge
- Dashboard: billing summary widget (outstanding total, overdue count, next milestone)

**Chunk 7 -- Milestone Management:**
- `PATCH /api/projects/[id]/milestones/[milestoneId]` -- update scheduled date/status (pending only)
- Project cancellation cascades: cancels pending/draft_created milestones + linked draft invoices

**New files (20):**
- `src/lib/billing.ts`, `src/lib/billing-validation.ts`, `src/lib/invoice-utils.ts`
- `src/lib/invoice-email.ts`, `src/lib/cron-auth.ts`, `src/lib/stripe.ts`
- `src/lib/email.ts` (modified -- added sendEmail helper)
- `src/types/billing.ts`
- `src/lib/__tests__/billing.test.ts`
- `src/app/api/invoices/[id]/send/route.ts`
- `src/app/api/invoices/[id]/pay/route.ts`
- `src/app/api/invoices/view/route.ts`
...
- `src/app/(standalone)/invoice/[token]/page.tsx`
- `src/components/billing/InvoiceView.tsx`
- `src/components/admin/BillingTimeline.tsx`

**Modified files (10):**
- `src/lib/schema.ts`, `src/lib/env.ts`, `.env.example`
- `src/app/api/proposals/respond/route.ts`
- `src/app/api/invoices/route.ts`, `src/app/api/invoices/[id]/route.ts`
- `src/app/api/projects/[id]/route.ts`
- `src/app/admin/projects/[id]/page.tsx`, `src/components/admin/ProjectDetailView.tsx`
- `src/components/admin/InvoiceDetailView.tsx`
- `src/app/admin/page.tsx`

**Document reorganization:**
- All governance docs moved from `docs/` to `.claude/` (HANDOFF, AGENT-PERSONAS, AUDIT-MAP, DIRECTORY-STRUCTURE, DOCUMENT-INDEX, SITEMAP, TESTING-PLAN, superpowers specs/plans)
- Sessions 29-51 archived to `.claude/archive/archived.md`
- CLAUDE.md references updated to `.claude/` paths
- `docs/` directory removed

**274 tests passing (16 new billing tests). Build clean.**

**VPS state:** Running commit `420611f`. Many commits pending deploy. MUST run Drizzle migration on production DB before deploying.

**Deploy checklist:**
1. Add `CRON_SECRET` to `/var/www/builtbybas/.env.local` (generate with `openssl rand -hex 32`)
2. Run `pnpm drizzle-kit push` or apply migration SQL to production DB
3. Deploy: `cd /var/www/builtbybas && git pull && pnpm install --frozen-lockfile && pnpm build && pm2 restart builtbybas`
4. Add crontab: `0 13 * * * source /var/www/builtbybas/.cron.env && curl -s -X POST https://builtbybas.com/api/cron/billing -H "Authorization: Bearer $CRON_SECRET"`
5. Store CRON_SECRET in `/var/www/builtbybas/.cron.env` (chmod 600)

---

## Session 56 Changes (2026-03-15)

**Milestone Billing System -- Design Complete:**
- Full spec written and reviewed: `.claude/superpowers/specs/2026-03-14-milestone-billing-design.md`
- 50/25/25 billing model: deposit on proposal acceptance, midpoint at client meeting, final after delivery
- Hybrid architecture: event-driven deposit (instant on proposal accept) + daily cron for midpoint/final
- Stripe framework built but dormant (activated by adding env vars)
- Cron worker for overdue detection, payment reminders, milestone alerts
- Client-facing invoice page with token-based access (same pattern as proposals)
- Invoice email system with branded templates
- Admin UI: billing timeline on projects, send/resend/mark-paid on invoices, billing dashboard widget
- Spec passed code review with all critical/important issues addressed

**Bug fix: Invoice form not showing projects:**
- `src/app/admin/invoices/new/page.tsx` -- was filtering to only `in_progress` projects
- Now shows `planning`, `in_progress`, and `on_hold` projects
- Root cause: projects default to `planning` status, so newly created projects were invisible

**Proposal page policy footer:**
- Added 5 policy links to proposal response page footer (Privacy, Terms, Cookie, Refund, Responsible AI)
- Policies open in animated on-screen overlay with Framer Motion (slide up, spring physics, exit animation)
- Close via X button, Escape key, or click outside
- New files: `src/components/proposal/ProposalPolicyOverlay.tsx`, `src/components/proposal/ProposalPolicies.tsx`

**258 tests pass, build clean.**

**VPS state:** Running commit `420611f`. Multiple commits pending deploy (Zod fix, invoice form fix, proposal policies, billing spec).

---

## Session 55 Changes (2026-03-14)

**Intake form confirmed working on production.** Bas tested the live site and the form submits successfully. The CRITICAL status from Session 54 is resolved -- the diagnostic improvements from that session (field-level error reporting, Suspense boundary, CSP split) appear to have addressed the issue.

**Zod import standardization fix:**
- `src/lib/intake-validation.ts` -- changed `import { z } from "zod"` to `import { z } from "zod/v4"`
- `src/app/api/intake/[id]/status/route.ts` -- changed `import { z } from "zod"` to `import { z } from "zod/v4"`
- These 2 files were missed during Session 52 Wave 2 (which standardized all other Zod imports to `zod/v4`)
- 258 tests pass, build clean

**Known non-issue (carried forward):**
- CSP `eval()` warning in Chrome Issues tab -- Next.js polyfill bundle contains `Function("return this")()` as a globalThis fallback. Never executes in modern browsers. Cosmetic only.

**VPS state:** Running commit `420611f`. Needs redeploy with Zod import fix.

---

## Session 54 Changes (2026-03-14)
...
- Added client-side `fullIntakeSchema` pre-submission validation in `useIntakeForm.ts`
- Changed radio/checkbox group labels from `<Label>` to `<GroupLabel>` (accessibility fix)
- Added `src/lib/__tests__/intake-full-validation.test.ts`
- Commits: `6badc61`, `420611f`
...
Sessions 29-51 (2026-03-06 through 2026-03-14) have been archived to `.claude/archive/archived.md`.
