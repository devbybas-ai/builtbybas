...
import { services, appointments } from "@/data/seed";
import {
  staggerContainer,
  staggerItem,
  fadeUp,
...
function AptCard({ apt }: AptCardProps) {
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mt-1.5">
            ...
            <span className="text-[10px] text-white/30">{formatDate(apt.date)}</span>
          </div>
        </div>
        ...
        <div className="flex-shrink-0 flex flex-col gap-1">
          {apt.status === "confirmed" && (
            <GlassButton variant="secondary" size="sm" icon={RefreshCw}>
              Reschedule
            </GlassButton>
          )}
          {apt.status === "pending" && (
            <>
              <GlassButton variant="primary" size="sm" icon={Check}>
                Confirm
              </GlassButton>
              <GlassButton variant="danger" size="sm" icon={X}>
                Cancel
              </GlassButton>
            </>
          )}
          {apt.status === "cancelled" && (
            <GlassButton variant="ghost" size="sm" icon={RefreshCw}>
              Rebook
            </GlassButton>
          )}
        </div>
  ...
  return (
    <PageWrapper>
      ...
      <PageHeader
        ...
        color="emerald"
        action={
          <GlassButton variant="primary" icon={CalendarDays}>
            New Appointment
          </GlassButton>
        }
      />
      ...
      >
        <StatCard label="Today's Appointments" value={2} icon={Calendar}    color="emerald" />
        <StatCard label="This Week"             value={4} icon={CalendarDays} color="cyan" />
        <StatCard label="Revenue Scheduled"     value="$1,275" icon={DollarSign} color="violet" />
        <StatCard label="Pending Confirm"       value={1} icon={Clock}       color="amber" />
      </motion.div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-5">
        ...
        <div className="lg:col-span-3 space-y-4">
          ...
          {/* Filter tabs */}
          <motion.div variants={fadeUp} initial="hidden" animate="visible" className="glass rounded-xl p-1 flex gap-1">
            {TABS.map((tab) => (
              <button
                key={tab}
          ...
          <AnimatePresence mode="wait">
            ...
            >
              {filtered.length === 0 ? (
                ...
                filtered.map((apt) => <AptCard key={apt.id} apt={apt} />)
              )}
            </motion.div>
          </AnimatePresence>
        </div>
            ...
            <AnimatePresence mode="wait">
              {submitted ? (
                ...
                >
                  ...
                  <GlassButton variant="secondary" size="sm" onClick={() => { setSubmitted(false); setFormData({ name: "", email: "", date: "", time: "", notes: "" }); }}>
                    Book Another
                  </GlassButton>
                </motion.div>
              ) : (
                ...
                >
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full glass rounded-lg px-3 py-2 text-sm text-white placeholder-white/20 outline-none focus:border-emerald-400/30 transition-colors"
                  ...
                  <div>
                    ...
                    <div
                      className="w-full glass rounded-lg px-3 py-2 text-sm text-emerald-400 font-medium"
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full glass rounded-lg px-3 py-2 text-sm text-white/70 outline-none focus:border-emerald-400/30 transition-colors"
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className="w-full glass rounded-lg px-3 py-2 text-sm text-white/70 outline-none focus:border-emerald-400/30 transition-colors bg-transparent"
                      style={{ border: "1px solid rgba(255,255,255,0.08)", colorScheme: "dark" }}
                    >
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      rows={2}
                      className="w-full glass rounded-lg px-3 py-2 text-sm text-white placeholder-white/20 outline-none focus:border-emerald-400/30 transition-colors resize-none"
                      style={{ border: "1px solid rgba(255,255,255,0.08)" }}
                    />
                  </div>
