...
import { demoFormatCurrency } from "@/lib/demo-utils";
import { inventoryItems } from "@/data/demo-seed";
...
export function InventoryDemo() {
  const [items, setItems] = useState(inventoryItems.map((i) => ({ ...i })));
  ...
  function handleReorder(id: string) {
    setItems((prev) =>
      prev.map((item) =>
        item.id === id
          ? { ...item, status: "in-stock", qty: item.minQty * 2 }
          : item
      )
    );
    showToast("Reorder placed — stock replenished");
  }
  ...
  function handleAddItem() {
    const newItem = {
      ...
      name: "New Item (Demo)",
      sku: `DEMO-${Date.now().toString(36).toUpperCase()}`,
  ...
  return (
    <DemoPageWrapper>
      ...
      >
        <DemoStatCard label="Total SKUs" value={items.length} icon={Package} color="violet" />
        <DemoStatCard label="In Stock" value={inStockCount} icon={CheckCircle} color="emerald" />
        <DemoStatCard
          label="Low Stock"
          value={lowStockCount}
          icon={AlertTriangle}
        ...
        />
        <DemoStatCard label="Out of Stock" value={outOfStockCount} icon={XCircle} color="rose" />
      </motion.div>
      ...
      <motion.div variants={demoFadeUp} className="flex flex-col sm:flex-row gap-3 mb-5">
        ...
        <DemoGlassButton variant="primary" icon={Plus} onClick={handleAddItem}>
          Add Item
        </DemoGlassButton>
      ...
      {/* Header Row */}
      <motion.div variants={demoFadeUp} className="grid grid-cols-12 gap-3 px-4 mb-2">
              ...
              >
                ...
                <div className="col-span-2 text-right">
                  <span className="text-sm text-white/70 font-medium">
                    {demoFormatCurrency(item.unitCost)}
                  </span>
                </div>
                ...
                <div className="col-span-1 flex justify-end">
                  {(item.status === "low-stock" || item.status === "out-of-stock") && (
                    <DemoGlassButton variant="primary" size="sm" icon={Zap} onClick={() => handleReorder(item.id)}>
                      Reorder
                    </DemoGlassButton>
                  )}
                </div>
